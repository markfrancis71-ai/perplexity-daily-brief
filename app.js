...
/* ===== DAILY LAB PERSISTENCE (localStorage only — static version) ===== */
(function() {
  var LAB_FIELDS = [
    'lab-intent',
    'lab-block1-artifact', 'lab-block1-tools',
    'lab-block2-friction', 'lab-block2-checklist',
    'lab-block3-device', 'lab-block3-experiment',
    'lab-wrapup-built', 'lab-wrapup-test', 'lab-wrapup-backlog'
  ];

  function getDateKey() {
    var d = new Date();
    return 'lab-' + d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
  }
  ...
  function loadLab() {
    ...
    try {
      ...
      var areas = document.querySelectorAll('.lab-note-area');
      if (!areas.length) return;
      LAB_FIELDS.forEach(function(f, i) {
        if (areas[i] && data[f]) areas[i].textContent = data[f];
      });
    } catch(e) {}
  }
  ...
  function saveLab() {
    ...
    var data = {};
    LAB_FIELDS.forEach(function(f, i) {
      if (areas[i]) data[f] = areas[i].textContent.trim();
    });
  ...
  function updateStatus(msg, synced) {
    ...
    if (!el) return;
    el.textContent = msg;
    el.className = 'lab-status' + (synced ? ' lab-synced' : '');
  }
  ...
  function initLab() {
    ...
    areas.forEach(function(area) {
      area.addEventListener('input', function() {
        saveLab();
        updateStatus('Saved locally', false);
      });
    });
    // Save to Google Sheets via Apps Script + localStorage
    ...
    if (btn) {
      ...
      btn.addEventListener('click', function() {
        ...
        var payload = {
          ...
          intent: data['lab-intent'] || '',
          block1_artifact: data['lab-block1-artifact'] || '',
          block1_tools: data['lab-block1-tools'] || '',
          block2_friction: data['lab-block2-friction'] || '',
          block2_checklist: data['lab-block2-checklist'] || '',
          block3_device: data['lab-block3-device'] || '',
          block3_experiment: data['lab-block3-experiment'] || '',
          wrapup_built: data['lab-wrapup-built'] || '',
          wrapup_test: data['lab-wrapup-test'] || '',
          wrapup_backlog: data['lab-wrapup-backlog'] || '',
        };
        btn.disabled = true; btn.textContent = 'Saving...';
        updateStatus('Syncing to Google Sheets...', false);
        fetch('https://script.google.com/macros/s/AKfycbxGgo6h2pGUNWc0NNQSqIzBYBBBLndPNXI56h9hg_LtPNu66bSKY5wjrvXYxySw7DOR/exec', {
          method: 'POST',
        ...
        .then(function(res) {
          if (res.success) {
            updateStatus('Saved to Google Sheets', true);
            btn.textContent = 'Saved';
            setTimeout(function() { btn.disabled = false; btn.textContent = 'Save to Sheets'; }, 2000);
          } else {
            updateStatus('Save failed: ' + (res.error || 'unknown'), false);
            btn.disabled = false; btn.textContent = 'Save to Sheets';
          }
        })
        .catch(function(err) {
          updateStatus('Save failed -- check connection', false);
          btn.disabled = false; btn.textContent = 'Save to Sheets';
        });
      });
    }
